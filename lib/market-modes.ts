...
/**
 * Deployed escrow generation, as this module models it. `squad_pool` needs 2,
 * meaning a real two-sided escrow. `contracts-soroban/mimir-squad` implements
 * one, but this axis describes the `mimir-market` contract a claim actually
 * lives in — a claim there is still one-sided, so it stays 1 until the squad
 * contract is the escrow behind a claim rather than a separate surface.
 */
