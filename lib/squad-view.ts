 ...
 * the point — the numbers below describe a claim in `mimir-market` as deployed,
 * where:
 *
 ...
 *
 * V1 (real two-sided deposits, side shares, proportional payout) needs a real
 * two-sided escrow and is gated behind it; see SETTLEMENT_MODE_POLICY.squad_pool.
 * `contracts-soroban/mimir-squad` is that escrow, read through `lib/squad-pool.ts`
 * — but a claim in `mimir-market` is not backed by it, which is exactly why this
 * module has to keep saying so.
 */
...
export interface SquadSide {
  ...
  id: "A" | "B";
  label: string;
  /** Total committed to this side, in USDC atomic units (7 decimals). */
  stakeUnits: bigint;
...
export function buildSquadView(input: SquadInput): SquadView {
  ...
  // Deduplicated so the same address arriving twice from two reads does not stack
  // two avatars for one participant.
  //
  // Trim only — NOT `toLowerCase()`. A Stellar strkey is case-SENSITIVE base32, so
  // folding case here would emit an address that is no longer valid: the avatar's
  // title, its key and anything a caller links it to would all be corrupt. It also
  // buys nothing, because a strkey has no lower-case form to collapse.
    ...
    sides: [
      {
        ...
        avatars: input.creator ? [input.creator.trim()] : [],
