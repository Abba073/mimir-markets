...
function eventIsComplete(row: ExportedAnalyticsEvent): boolean {
  ...
  // `network` is a string since envelope v2 (it was the numeric `chain_id`), so
  // completeness checks the type the current envelope actually carries.
  if (!finiteNumber(properties.event_version) || typeof properties.network !== "string") return false;
