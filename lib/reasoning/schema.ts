...
export function validateReasoningEvent(event: ReasoningEvent): ValidationResult {
  ...
  // The feed shows a registry identity, not a raw wallet. Matched as a Stellar
  // strkey (`G…` account or `C…` contract): the EVM form this replaced could
  // never fire, so a raw wallet would have been published into the public feed.
  if (/^[GC][A-Z2-7]{55}$/.test(event.agentId.trim())) {
