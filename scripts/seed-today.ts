/**
 * Seed today's live-event markets, signed with the market-creator's Stellar
 * keypair. Edit SEEDS with current fixtures/prices, then:
 *
 *   DRY_RUN=1 npx tsx --env-file-if-exists=.env.local scripts/seed-today.ts   # preview
 *   npx tsx --env-file-if-exists=.env.local scripts/seed-today.ts             # create
 *
...
async function main() {
  ...
  console.log(
    `Seeding ${live.length} claims on ${contractId}` +
      `${skipped ? ` (${skipped} skipped — deadline too close/past)` : ""}`,
  );
  ...
  console.log(
    `Fees: ${(balances.xlm ?? 0).toFixed(4)} XLM · ` +
      `USDC: ${balances.usdc === null ? "no trustline" : balances.usdc.toFixed(4)} · ` +
      `need ~${live.length * STAKE_USDC} USDC\n`,
  );
  if (!DRY_RUN) {
    if (!balances.exists) throw new Error("creator account does not exist — run npm run agents:fund");
    if (balances.usdc === null) {
      throw new Error("creator holds no USDC trustline — run npm run agents:fund");
    }
    if (balances.usdc < live.length * STAKE_USDC) {
      throw new Error("insufficient creator USDC — run npm run agents:fund");
    }
  }

  let created = 0;
  for (const s of live) {
    const deadline = Math.floor(new Date(s.deadlineIso).getTime() / 1000);
    ...
    try {
      const result = await createClaim(creator.signer, {
        question:         s.question,
        creator_position: s.a,
        counter_position: s.b,
        resolution_url:   s.url,
        deadline,
        stake_amount:     STAKE_USDC,
        category:         s.category,
        market_type:      "binary",
        odds_mode:        "pool",
        settlement_rule:  s.rule,
        max_challengers:  100,
        visibility:       "public",
      });
      created++;
