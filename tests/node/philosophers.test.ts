...
test("no philosopher slug collides with the classic council", () => {
  // Wallet env vars are COUNCIL_<SLUG>_SECRET across BOTH tracks, so a
...
test("wallet env names are derived deterministically from the slug", () => {
  assert.equal(philosopherSecretEnv("lao-tzu"), "COUNCIL_LAO_TZU_SECRET");
  assert.equal(philosopherPublicEnv("lao-tzu"), "COUNCIL_LAO_TZU_PUBLIC");
  assert.equal(philosopherSecretEnv("socrates"), "COUNCIL_SOCRATES_SECRET");
});
...
test("no philosopher shares a slug with a classic persona", () => {
  // Both tracks derive COUNCIL_<SLUG>_SECRET from the slug, so a collision
...
test("every persona across both tracks maps to a distinct key env", () => {
  const envs = [
    ...COUNCIL_PERSONAS.map((p) => personaSecretEnv(p)),
    ...PHILOSOPHER_PERSONAS.map((p) => philosopherSecretEnv(p.slug)),
  ];
  assert.equal(new Set(envs).size, envs.length);
});

test("a hyphenated slug becomes a valid env name", () => {
  // lao-tzu must not produce COUNCIL_LAO-TZU_SECRET, which no shell exports.
  assert.equal(philosopherSecretEnv("lao-tzu"), "COUNCIL_LAO_TZU_SECRET");
  for (const persona of PHILOSOPHER_PERSONAS) {
    assert.match(philosopherSecretEnv(persona.slug), /^COUNCIL_[A-Z0-9_]+_SECRET$/);
  }
});
