...
test("fractional USDC previews stay exact at 7 decimals", () => {
  const preview = previewChallengerPayout({
    settlementMode: "pool",
    stake: 0.5,
    creatorStake: 1.25,
  ...
  // Atomic units are now 10^-7 (Stellar Asset Contract), so the same market rounds
  // one decimal place further in:
  // pool after join = 0.75 → share = floor(5000000 * 12500000 / 7500000) = 8333333
  assert.equal(preview.totalReturn, 1.3333333);
  assert.equal(preview.netProfit, 0.8333333);
});
