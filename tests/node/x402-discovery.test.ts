...
const POLICY: DiscoveryPolicy = {
  maxPriceUnits: 100_000n, // $0.01 at 7 decimals
...
test("a price we cannot parse is a refusal, not a guess", () => {
  ...
  // nobody read.
  // `$0.00000001` is an EIGHTH decimal place: not representable in atomic USDC at
  // any precision Stellar offers, so it stays a refusal. `$0.0000001` (seven) is
  // now representable and is asserted as parseable below.
  for (const price of ["free", "0.001 ETH", "1e-3", "$0.00000001", "-1", "$$1", ""]) {
    assert.equal(
...
test("prices parse to USDC atomic units at 7 decimals", () => {
  // Every figure here is ten times its former value: Stellar USDC's SAC exposes 7
  // decimals where the ERC-20 had 6.
  assert.equal(parseAdvertisedPrice("$0.001"), 10_000n);
  assert.equal(parseAdvertisedPrice("1"), 10_000_000n);
  assert.equal(parseAdvertisedPrice("$0"), 0n);
  assert.equal(parseAdvertisedPrice("0.000001"), 10n);
  // The smallest representable amount — one atomic unit, 0.00000001 of a dollar
  // short of nothing. This was unparseable at 6 decimals.
  assert.equal(parseAdvertisedPrice("0.0000001"), 1n);
...
test("the admitted price is returned so the caller caps the actual payment", () => {
  ...
  assert.equal(verdict.priceUnits, 50_000n);
});
