...
test("the waiver matches addresses exactly, because StrKeys are case-sensitive", () => {
  // Stellar addresses are base32 StrKeys, not case-insensitive hex: changing the
  // case produces a DIFFERENT (and invalid) address. The EVM version of this test
  // asserted the opposite, because a checksummed `0x…` address really was the same
  // address. Folding case here would waive a fee for someone who is not the earner.
  const result = split({ earner: AGENT_OWNER.toUpperCase(), basketCreatorRecipient: null });
  assert.equal(result.waived.includes("agent_owner"), false, "a different string is a different address");
  assert.ok(result.agentOwnerFeeUnits > 0n, "the agent owner leg is still charged");

  // Padding is not part of an address, so it is still trimmed away.
  const padded = split({ earner: ` ${AGENT_OWNER} `, basketCreatorRecipient: null });
  assert.equal(padded.agentOwnerFeeUnits, 0n, "surrounding whitespace is not an address difference");
});
