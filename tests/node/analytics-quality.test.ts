...
function row(event: string, complete = true): ExportedAnalyticsEvent {
  // Envelope v2: `network` (a string) replaced the numeric `chain_id`.
  return { event, properties: complete
    ? { event_version: 2, network: "testnet", settlement_mode: "pool", is_internal: false }
    : { event_version: 2, network: "testnet", is_internal: false } };
}
