...
/** Env var holding this philosopher's Stellar secret seed (`S…`). */
export function philosopherSecretEnv(slug: string): string {
  return `COUNCIL_${philosopherEnvSlug(slug)}_SECRET`;
}

/** Env var holding this philosopher's `G…` public key. Web-server safe. */
export function philosopherPublicEnv(slug: string): string {
  return `COUNCIL_${philosopherEnvSlug(slug)}_PUBLIC`;
}
